 ...
 */
  public static String approveDeny(final int argLeaveId,
    ...
    if (ed != null) {
    int leavAvail = 0;
    LeaveType t = ld.getLeaveType();
    if (LeaveType.ML == t) {
    leavAvail = ed.getEmpML();
    }
    else if (LeaveType.PL == t) {
    leavAvail = ed.getEmpPL();
    }
    else if (LeaveType.EL == t) {
    leavAvail = ed.getEmpEL();
    }
      // int leavAvail = ed.getEmpML();
      // int leavAvail = ed.getEmpPL();
      // int leavAvail = ed.getEmpEL();
      // int leavAvail = 0;
...
public static String approveDenyforHR(final int argLeaveId,
final int argEmpManagerId,
final String argLeaveStatus,
final String argLeaveMgrComments) {
String res = "";
...
leavAvail = leavAvail + noDays;
Employee e = edao().findHr();
int hrId = e.getEmpId();
String leaveStatus = "";
if(LeaveType.ML!= t) {
  res = "Holiday nhi mil ri h kaam kr ";

} else {
if (hrId != argEmpManagerId) {
  res = "you are unauthorised to approve the Leave";
} else {
  if (argLeaveStatus.equals("YES")) {
    res = "Leave Approved Successfully...";
 ...
 */
  public static String applyLeave(final String argLeaveStartDate,
    ...
    String date[] = {"2019-08-15","2019-12-25"};
     for(int i = 0; i<date.length ;i++)  {
    Date odate = sdf.parse(date[i]);
    ...
    if (e != null ) { 
    int leavAvail = 10;
    if (LeaveType.ML == argLeaveType) {
    leavAvail = e.getEmpML();
    }
    else if (LeaveType.PL == argLeaveType) {
    leavAvail = e.getEmpPL();
    }
    else if (LeaveType.EL == argLeaveType) {
    leavAvail = e.getEmpEL();
    }
