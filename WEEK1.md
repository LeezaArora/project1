...
IdentityFile ~/.ssh/id_rsa
  ...
  ** Clone repository at Gitbash **
    ...
    * `pwd` -- Check and verify that the current working directory is C:\users\Hvuser/workspace
    * `git clone ssh://git-codecommit.us-east-2.amazonaws.com/v1/repos/FTP116 `
    * `cd FTP116`
    * `git status`
  * open Visual Studio (VS) Code
    * Open folder c:\users\Hvuser\workspace\FTP116
...
# Workshop material - Day #2 - MySQL Workbench/Cli/Curl/Browser to interact w/ application
  ...
  * `CREATE DATABASE FTP116;` and click the lightning button
  * `CREATE USER 'FTP116'@'localhost' IDENTIFIED BY 'FTP116';`
  * `GRANT ALL ON FTP116.* TO 'FTP116'@'localhost';`
  ...
  * go to gitbash, ensure you are in workspace/FTP116
  * `cd restservice/leavemanager`
  * `mvn compile`
  * `mvn exec:java -Dexec.mainClass=com.hexaware.FTP116.util.CliMain`
  ...
  * Build the war file as follows:
    ...
    * `cp target/FTP116-0.0.1-SNAPSHOT.war /D/FTP/apache-tomcat-8.5.16-windows-x64/apache-tomcat-8.5.16/webapps/FTP116.war`
    * start tomcat and tail its logs
      * `cd D/FTP/apache-tomcat-8.5.16-windows-x64/apache-tomcat-8.5.16`
      * `rm -rf logs/*`
      * `./bin/startup.sh`
      * `tail -f logs/*`
      * `curl -vvv http://localhost:8080/FTP116/api/employees | python -m json.tool`
      * `curl -vvv http://localhost:8080/FTP116/api/employees/2000 | python -m json.tool`
   * `cd ../../webui/lm-app/`
   * `npm install`
   * `ng build`
   * `cp ../../restservice/leavemanager/target/FTP116-0.0.1-SNAPSHOT.war ./FTP116.war`
   * `cd dist`
   * `jar -uvf ../FTP116.war *`
   * `cd ..`
   * `jar -tvf FTP116.war`
   * `cp FTP116.war /D/FTP/apache-tomcat-8.5.16-windows-x64/apache-tomcat-8.5.16/webapps/FTP116.war`
   * Notice that the tail terminal shows that the new version of the web application archive is now getting deployed
   * Open Chrome browser and navigate to http://localhost:8080/FTP116/
...
# Workshop material - Day #4 - Configure Project Board in Trello for  Managing and Tracking Your Project
  ...
  * Create a board for your project as "FTP116"
  ...
  * [Team member #3]
     * Create "Current Sprint" with "FTP116 Sprint 1" as label 
